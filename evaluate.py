...
if __name__ == '__main__':
    ...
    if args.net == 'basenet':
        ...
        pred_dec = network.mvod_bottleneck_lstm1.SSD(num_classes=num_classes, alpha=args.width_mult, is_test=True,
        ...
        pred_dec = network.mvod_bottleneck_lstm2.SSD(num_classes=num_classes, alpha=args.width_mult, is_test=True,
                                                     config=config, batch_size=1)
        ...
        pred_dec = network.mvod_bottleneck_lstm3.SSD(num_classes=num_classes, alpha=args.width_mult, is_test=True,
                                                     config=config, batch_size=1)
        ...
        pred_dec = network.mvod_lstm4.SSD(num_classes=num_classes, alpha=args.width_mult, is_test=True, config=config,
                                          batch_size=1)
        ...
        pred_dec = network.mvod_lstm5.SSD(num_classes=num_classes, alpha=args.width_mult, is_test=True, config=config,
                                          batch_size=1)
    ...
    for i in range(len(dataset)):
        ...
        if plot_image:
            ...
            for i, box in enumerate(boxes):
                x1 = box[0]
                y1 = box[1]
                image = cv2.rectangle(img_draw, (x1, y1), (box[2], box[3]), (36, 255, 12), 2)
                cv2.putText(image, dataset._classes_names[labels[i]], (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                            (36, 255, 12), 2)
